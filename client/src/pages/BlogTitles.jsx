  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setContent(rawText)
        const parsedTitles = Array.isArray(data.titles)
          ? data.titles.map((item) => String(item || '').trim()).filter(Boolean)
          : []

        setTitles(parsedTitles.length > 0 ? parsedTitles : [rawText.trim() || 'No titles generated yet'])
      } else {
        ...
        {!content ? (
          ...
          <div className='mt-3 text-sm h-full overflow-y-scroll text-slate-600'>
            {titles.length > 0 ? (
              ...
              <div className='text-xs text-gray-500'>No titles parsed from the AI output.</div>
            )}
          </div>
        )}
      </div>
